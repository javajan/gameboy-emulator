...
void LDHL_SP_N(gameboy_t* gameboy) {
	...
	gb_flag_set(gameboy, GB_FLAG_ZERO, false);
...
void PUSH(gameboy_t* gameboy, uint16_t r1) {
	...
	gb_mem_write(&gameboy->memory, gameboy->cpu.sp, (uint8_t)(r1 & 0xFF));
...
void POP_AF(gameboy_t* gameboy) {
	POP(gameboy, &gameboy->cpu.af);
	gameboy->cpu.f &= 0xF0;
}
...
void ADD_A_N(gameboy_t* gameboy) {
	ADD_R1_R2(gameboy, &gameboy->cpu.a, gb_mem_read(&gameboy->memory, gameboy->cpu.pc));
	gameboy->cpu.pc++;
}
...
void ADC_R1_R2(gameboy_t* gameboy, uint8_t* r1, uint8_t r2) {
	...
	int nibbleAdd = ((*r1) & 0x0F) + (r2 & 0x0F) + carry;
	int byteAdd = *r1 + r2 + carry;
...
void ADC_A_N(gameboy_t* gameboy) {
	ADC_R1_R2(gameboy, &gameboy->cpu.a, gb_mem_read(&gameboy->memory, gameboy->cpu.pc));
	gameboy->cpu.pc++;
}

// Subtracts r2 from r1
void SUB_R1_R2(gameboy_t* gameboy, uint8_t* r1, uint8_t r2) {
	uint16_t nibbleSub = ((*r1) & 0x0F) - (r2 & 0x0F);
...
void SUB_A_N(gameboy_t* gameboy) {
	SUB_R1_R2(gameboy, &gameboy->cpu.a, gb_mem_read(&gameboy->memory, gameboy->cpu.pc)); 
	gameboy->cpu.pc++; 
}
...
void SBC_R1_R2(gameboy_t* gameboy, uint8_t* r1, uint8_t r2) {
	...
	uint16_t nibbleSub = ((*r1) & 0x0F) - (r2 & 0x0F) - carry;
...
void ADD_R1_R2_16Bit(gameboy_t* gameboy, uint16_t* r1, uint16_t r2) {
	uint16_t lowAdd = ((*r1) & 0x0FFF) + (r2 & 0x0FFF);
...
void ADD_HL_BC(gameboy_t* gameboy) { ADD_R1_R2_16Bit(gameboy, &gameboy->cpu.hl, gameboy->cpu.bc); }
void ADD_HL_DE(gameboy_t* gameboy) { ADD_R1_R2_16Bit(gameboy, &gameboy->cpu.hl, gameboy->cpu.de); }
void ADD_HL_SP(gameboy_t* gameboy) { ADD_R1_R2_16Bit(gameboy, &gameboy->cpu.hl, gameboy->cpu.sp); }
...
void AND_A_N(gameboy_t* gameboy) {
	AND_R1_R2(gameboy, &gameboy->cpu.a, gb_mem_read(&gameboy->memory, gameboy->cpu.pc));
	gameboy->cpu.pc++;
}
...
void OR_A_N(gameboy_t* gameboy) { 
	OR_R1_R2(gameboy, &gameboy->cpu.a, gb_mem_read(&gameboy->memory, gameboy->cpu.pc)); 
	gameboy->cpu.pc++;
}
...
void XOR_A_N(gameboy_t* gameboy) {
	XOR_R1_R2(gameboy, &gameboy->cpu.a, gb_mem_read(&gameboy->memory, gameboy->cpu.pc));
	gameboy->cpu.pc++;
}
...
void CMP_A_N(gameboy_t* gameboy) { 
	uint8_t tmp = gameboy->cpu.a;
	SUB_A_N(gameboy);
	gameboy->cpu.a = tmp;
}
...
void DAA(gameboy_t* gameboy) {
	if (!gb_flag_get(*gameboy, GB_FLAG_NEGATIVE)) { 
		// after an addition, adjust if (half-)carry occurred or if result is out of bounds
		if (gb_flag_get(*gameboy, GB_FLAG_CARRY) || gameboy->cpu.a > 0x99) { 
			gameboy->cpu.a += 0x60;
			gb_flag_set(gameboy, GB_FLAG_CARRY, true);
		}
		if (gb_flag_get(*gameboy, GB_FLAG_HALF_CARRY) || (gameboy->cpu.a & 0x0f) > 0x09) {
			gameboy->cpu.a += 0x6;
		}
	}
		...
		if (gb_flag_get(*gameboy, GB_FLAG_HALF_CARRY)) {
			gameboy->cpu.a -= 0x6;
		}
	}
...
void RLCA(gameboy_t* gameboy) {
	gameboy->cpu.a = (gameboy->cpu.a << 1 | gameboy->cpu.a >> 7);
	
	gb_flag_set(gameboy, GB_FLAG_ZERO, false);
	gb_flag_set(gameboy, GB_FLAG_NEGATIVE, false);
	gb_flag_set(gameboy, GB_FLAG_HALF_CARRY, false);
	gb_flag_set(gameboy, GB_FLAG_CARRY, (gameboy->cpu.a & 0x1) == 1);
}
...
void RLA(gameboy_t* gameboy) {
	...
	gb_flag_set(gameboy, GB_FLAG_CARRY, (gameboy->cpu.a & 0x80) == 0x80);

	gameboy->cpu.a = (gameboy->cpu.a << 1) + carry;
...
void RRA(gameboy_t* gameboy) {
	...
	gb_flag_set(gameboy, GB_FLAG_CARRY, (gameboy->cpu.a & 0x1) == 0x1);
...
void JP_HL(gameboy_t* gameboy) { 
	gameboy->cpu.pc = gameboy->cpu.hl;
}

// jump absolute
void JP_NN(gameboy_t* gameboy) {
	uint8_t n1 = gb_mem_read(&gameboy->memory, gameboy->cpu.pc);
	uint8_t n2 = gb_mem_read(&gameboy->memory, gameboy->cpu.pc+1);
	uint16_t nn = (((uint16_t)n2) << 8) + n1;
	gameboy->cpu.pc = nn;
}
...
void JR(gameboy_t* gameboy) {
	gameboy->cpu.pc += (int8_t)gb_mem_read(&gameboy->memory, gameboy->cpu.pc) + 1;
}
...
void RLC_VAL(gameboy_t* gameboy, uint8_t* val) {
	*val = (*val << 1) | (*val >> 7);
	...
	gb_flag_set(gameboy, GB_FLAG_NEGATIVE, false);
	gb_flag_set(gameboy, GB_FLAG_HALF_CARRY, false);
	gb_flag_set(gameboy, GB_FLAG_CARRY, (*val & 1) == 1);
}
...
void RRC_VAL(gameboy_t* gameboy, uint8_t* val) {
	*val = (*val >> 1) | (*val << 7);
	...
	gb_flag_set(gameboy, GB_FLAG_NEGATIVE, false);
	gb_flag_set(gameboy, GB_FLAG_HALF_CARRY, false);
	gb_flag_set(gameboy, GB_FLAG_CARRY, ((*val) & 0x80) == 0x80);
}
