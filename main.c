#include <stdio.h>
#include <stdbool.h>
#include <string.h>

#ifdef __EMSCRIPTEN__
#include <emscripten.h>
#endif

#include "gameboy.h"


int main() {
	uint8_t boot[] = {  };

	gameboy_t gameboy;
	
	gb_init(&gameboy, boot, 256);
	
	//gb_run(&gameboy);
	
	//gb_print_registers(gameboy);
	
	//printf("%02x %02x\n", gb_mem_read(&gameboy, 0x1110), gb_mem_read(&gameboy, 0x1110)+1);
	
	/*for (int i=0; i<256; i++) {
		printf("\t{ NULL, 0, 0, NULL }, // 0x%02x\n", i);
	}*/
	
	for (int i=0; i<256; i++) {
		uint8_t opcode = boot[i];
		instruction_t instr = gb_base_instructions[opcode];
		
		if (instr.identifier != NULL) {
			char identifier[255];
			strcpy(identifier, instr.identifier);
			
			if (instr.immediates == 1) {
				sprintf(identifier, identifier, boot[i+1]);
			}
			else if (instr.immediates == 2) {
				sprintf(identifier, identifier, boot[i+2], boot[i+1]);
			}
		
			printf("%04x: \t\t %s\n", i, identifier);
		}
		else {
			printf("%04x: \t\t unknown instruction or raw data: $%02x\n", i, opcode);
			i++;
		}
		i += instr.immediates;
	}
	
	gb_stop(&gameboy);
	
	return 0;
}

